...
class Network_PSO(nn.Module):
	...
	def forward(self,params,localbcosts,posg,errg):
		errg_best = errg
		pos_best_g = posg
...
def backPropGrad(model,optimizer,pos_g_best,steps,X_train,Y_train,bsize=2048):
	criterion = nn.CrossEntropyLoss()
...
def PSO_grad(swarm,localbcosts,w,c1,c2,c3,maxiter,costfunc,verbose,logfile):
	...
	while (i<maxiter):
		...
		t_old = time.time()
		pos_best_g,err_best_g,swarm = costfunc(swarm,localbcosts,pos_best_g,err_best_g)
...
#		gradVal = findGrad(model,pos_best_g)
		gradVal = backPropGrad(model,optimizer,pos_best_g,1,X_train,Y_train)
		backpropsteps += 10
		swarm[:,2]*=w
		swarm[:,2] += c1*r1*(swarm[:,1]-swarm[:,0]) + c2*r2*(pos_best_g-swarm[:,0])-c3*(swarm[:,0]-gradVal) #new velocity
...
logfilePath = 'mnist.txt'
pos,err,iters,backpropsteps= PSO_grad(costfunc=model.forward,swarm=initial,maxiter=50,verbose=True,w = 0.9,c1=2.0,c2=2.0,c3=0.1,localbcosts=initialcosts,logfile =logfilePath)
...
print('number of back prop steps', backpropsteps)
