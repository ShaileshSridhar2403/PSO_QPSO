...
Xpath = 'GTEx_X_float64.npy'
Ypath = 'GTEx_Y_0-4760_float64.npy'
...
#			MAE_GTx = torch.sum(torch.sum(torch.abs(torch.sub(x,Y_train)),dim=0)/(2921*n_outputs))
			MAE_GTx = torch.sum(torch.sum(torch.abs(torch.sub(x,Y_train)),dim=0)/2921)/n_outputs #copy pasted from D-Gex file
...
#	initialcosts =  np.ones(num_particles)*-1
	...
	pos,err,iters=QPSO(costfunc=model.forward,swarm=initial,maxiter=1000,verbose=True,beta = 0.0000000000001,localbcosts=initialcosts,logfile =logfilePath)
	model.vecToMat(pos)
